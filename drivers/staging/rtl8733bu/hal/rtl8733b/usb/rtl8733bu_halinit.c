 ...
 *
 *****************************************************************************/
#define _RTL8733BU_HALINIT_C_

#include <hal_data.h>			/* HAL DATA */
#include "../../hal_halmac.h"		/* HALMAC API */
#include "../rtl8733b.h"		/* rtl8733b hal common define */
#include "rtl8733bu.h"
...
u8 rtl8733bu_fw_ips_init(_adapter *padapter)
{
...
u8 rtl8733bu_fw_ips_deinit(_adapter *padapter)
{
	struct sreset_priv *psrtpriv =  &GET_HAL_DATA(padapter)->srestpriv;
	struct debug_priv *pdbgpriv = &adapter_to_dvobj(padapter)->drv_dbg;
	struct pwrctrl_priv *pwrctl = adapter_to_pwrctl(padapter);
...
u32 rtl8733bu_init(PADAPTER padapter)
{
	u8 status = _SUCCESS;
...
u32 rtl8733bu_deinit(PADAPTER padapter)
{
	struct pwrctrl_priv *pwrctl = adapter_to_pwrctl(padapter);
	...
	status = rtl8733b_deinit(padapter);
	if (status == _FALSE) {
		RTW_INFO("%s: rtl8733b_hal_deinit fail\n", __func__);
		return _FAIL;
	}
...
u32 rtl8733bu_inirp_init(PADAPTER padapter)
{
...
void rtl8733bu_update_interrupt_mask(PADAPTER padapter, u8 bHIMR0 , u32 AddMSR, u32 RemoveMSR)
{
	...
	if (bHIMR0)
		rtw_write32(padapter, REG_HIMR0_8733B, *himr);
	else
		rtw_write32(padapter, REG_HIMR1_8733B, *himr);
...
void rtl8733bu_interface_configure(PADAPTER padapter)
{
	HAL_DATA_TYPE *pHalData = GET_HAL_DATA(padapter);
