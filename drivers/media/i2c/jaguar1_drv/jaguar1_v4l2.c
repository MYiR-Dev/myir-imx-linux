...
/* Channel selector (sysfs), defined in jaguar1_drv.c */
extern unsigned int jaguar1_active_ch;

/* Disabled: RKCIF hot-plug polling unused on i.MX */
/* #define WORK_QUEUE */
...
/* Rockchip-compatible DT properties (all optional, fall back to defaults). */
#define JAGUAR1_MODULE_INDEX			"rockchip,camera-module-index"
#define JAGUAR1_MODULE_FACING			"rockchip,camera-module-facing"
#define JAGUAR1_MODULE_NAME			"rockchip,camera-module-name"
#define JAGUAR1_MODULE_LENS_NAME		"rockchip,camera-module-lens-name"
...
/* Single source pad for one MIPI link. */
#define PAD_MAX					1
...
static const struct jaguar1_pixfmt jaguar1_formats[] = {
	{
		/* CSIS expects serial 16-bit YUV422 */
...
/* Active instance, for the live channel switch via module param. */
static struct jaguar1 *g_jaguar1;

/* Configure the chip to route the selected jaguar1_active_ch to the
 * single MIPI output. Re-runnable mid-stream for a live switch.
 */
static void jaguar1_apply_channels(struct jaguar1 *jaguar1)
{
	video_init_all video_init;
	enum NC_VIVO_CH_FORMATDEF fmt_idx = jaguar1->frame_size->fmt_idx;
	int ch;
	...
	for (ch = 0; ch < 4; ch++) {
		...
		video_init.ch_param[ch].interface =
			(ch == jaguar1_active_ch) ? YUV_422 : DISABLE;
	}
	jaguar1_start(&video_init);
	jaguar1_set_streaming(jaguar1, 1);
}

/* Called from the jaguar1_active_ch module param setter. */
void jaguar1_reconfig_active_ch(void)
{
	struct jaguar1 *jaguar1 = g_jaguar1;

	if (!jaguar1)
		return;
	mutex_lock(&jaguar1->mutex);
	if (jaguar1->streaming)
		jaguar1_apply_channels(jaguar1);
	mutex_unlock(&jaguar1->mutex);
}
...
static int jaguar1_stream(struct v4l2_subdev *sd, int on)
{
	...
	if (on) {
		...
		jaguar1_set_streaming(jaguar1, 0);
		jaguar1_stop();
		pm_runtime_put(&client->dev);
#ifdef WORK_QUEUE
...
static const struct v4l2_subdev_pad_ops jaguar1_subdev_pad_ops = {
	...
	.get_frame_interval = jaguar1_g_frame_interval,
...
static int jaguar1_probe(struct i2c_client *client)
{
	...
	/* Optional DT properties, fall back to defaults. */
	if (of_property_read_u32(node, JAGUAR1_MODULE_INDEX,
				 &jaguar1->module_index))
		jaguar1->module_index = 0;
	if (of_property_read_string(node, JAGUAR1_MODULE_FACING,
				    &jaguar1->module_facing))
		jaguar1->module_facing = "back";
	if (of_property_read_string(node, JAGUAR1_MODULE_NAME,
				    &jaguar1->module_name))
		jaguar1->module_name = "default";
	if (of_property_read_string(node, JAGUAR1_MODULE_LENS_NAME,
				    &jaguar1->len_name))
		jaguar1->len_name = "default";
