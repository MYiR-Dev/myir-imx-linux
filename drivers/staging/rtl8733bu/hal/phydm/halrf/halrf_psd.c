...
void _halrf_iqk_psd_init_8733b(void *dm_void,	 boolean onoff)
{
	...
	if (onoff) {
		...
		//DbgPrint("[PSD][8733B]iqkpsd init!\n");
	} else {
		...
		//DbgPrint("[PSD][8733B]iqkpsd resotre!\n");
	}
}
...
#if (DEV_BUS_TYPE == RT_USB_INTERFACE) || (DEV_BUS_TYPE == RT_SDIO_INTERFACE)
	if (dm->support_interface == ODM_ITRF_USB || dm->support_interface == ODM_ITRF_SDIO) {
		if (dm->support_ic_type & (ODM_RTL8822C | ODM_RTL8733B | ODM_RTL8814C | ODM_RTL8822E))
			delay_time = 2000;
		else
			delay_time = 0;
	}
	...
	u32 average)
{
	...
	if (mode == 2) {
		if (dm->support_ic_type & (ODM_RTL8822C | ODM_RTL8733B | ODM_RTL8814C |
		...
		if (reg_tmp == 0)
			...
			average_tmp = 32;
		if (!((dm->support_ic_type & ODM_RTL8733B)&&(dm->support_ic_type & ODM_RTL8730A)))
			odm_set_bb_reg(dm, R_0x1b1c, 0x000e0000, 0x0);
	}
...
halrf_psd_init(void *dm_void)
{
	...
	u32 backup_bb_reg_8733b[11] = {0x09f0, 0x09b4, 0x1c38, 0x1860, 0x1cd0,
	...
	if (psd->psd_progress) {
		...
		if (dm->support_ic_type & ODM_RTL8733B) {
			backup_bb_register(dm, bb_backup, backup_bb_reg_8733b, 11);
			...
			restore_bb_register(dm, bb_backup, backup_bb_reg_8733b, 11);
		}
#endif
	...
	RTL8198F_SUPPORT == 1 || RTL8197G_SUPPORT == 1)
		...
		if (!(dm->support_ic_type & 
			(ODM_RTL8822C | ODM_RTL8812F | ODM_RTL8814B | ODM_RTL8198F | ODM_RTL8197G |
			ODM_RTL8735B | ODM_RTL8730A | ODM_RTL8814C | ODM_RTL8733B | ODM_RTL8822E))) {
