...
/* Select active AHD input channel (0-3); applies live while streaming. */
void jaguar1_reconfig_active_ch(void);
unsigned int jaguar1_active_ch = 0;
static int jaguar1_set_active_ch(const char *val, const struct kernel_param *kp)
{
	int ret = param_set_uint(val, kp);

	if (ret)
		return ret;
	jaguar1_reconfig_active_ch();
	return 0;
}
static const struct kernel_param_ops jaguar1_active_ch_ops = {
	.set = jaguar1_set_active_ch,
	.get = param_get_uint,
};
module_param_cb(jaguar1_active_ch, &jaguar1_active_ch_ops,
		&jaguar1_active_ch, S_IRUGO | S_IWUSR);
